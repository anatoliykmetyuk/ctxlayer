...
|---------|---------|-----------|
| `ctx new [name]` | `newTask(name?, options?)` | ensureWorkspaceInitialized, supports `--task`, `--domain`, `--use-current-domain`, `--create-domain`, `--clone-from`, createTaskInDomain |
| `ctx import` | `importTask(options?)` | ensureWorkspaceInitialized, readConfigOrNull, supports `--domain` / `--task`, ensureTaskSymlink, set active when config empty/missing |
